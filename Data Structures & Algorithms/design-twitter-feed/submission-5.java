class Twitter {
    ...
    public List<Integer> getNewsFeed(int userId) {
        ...
        //     pq.add(new int[] {latestUserPost[1], latestUserPost[0], userPosts.size() - 1,
        //     userId});
        // }

        this.users.computeIfAbsent(userId, k -> new HashSet<>()).add(userId);
        for (Integer followee : this.users.get(userId)) {
            List<int[]> followeePosts = this.userPosts.computeIfAbsent(followee, k -> new ArrayList<>());
